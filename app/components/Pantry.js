...
// The signed-in user's pantry, stored at users/{uid}/inventory.
export default function Pantry({ user }) {
  ...
  const updateInventory = useCallback(async () => {
    try {
      const docs = await getDocs(collection(firestore, 'users', user.uid, 'inventory'));
      ...
      notify(
        error.code === 'permission-denied'
          ? 'Firestore rules are blocking access. Publish the rules in firestore.rules.'
          : 'Could not load your pantry. Check your Firebase configuration.',
        'error'
      );
    } finally {
      setLoading(false);
  ...
  const itemRef = (name) => doc(firestore, 'users', user.uid, 'inventory', name);
  ...
  const decrementItem = (item) => {
    const docRef = itemRef(item.name);
        ...
        <Stack spacing={3}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="space-between" alignItems={{ sm: 'flex-end' }}>
            ...
            <Stack direction="row" spacing={1.5} alignItems="center" sx={{ flexShrink: 0 }}>
              <Button variant="contained" size="large" startIcon={<AddIcon />} onClick={() => setDialogOpen(true)} sx={{ flexGrow: { xs: 1, sm: 0 } }}>
                Add item
              </Button>
              <UserMenu user={user} />
            </Stack>
